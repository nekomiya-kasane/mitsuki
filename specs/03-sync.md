...
> **Namespace**: `mitsuki::rhi` (sync primitives), `mitsuki::frame` (frame orchestration)
...
Integration with `01-window-manager.md` §6 cascade destruction protocol:

```
DestroyWindowCascade(parent):
  1. Get post-order descendant list: [grandchild, child, parent]
  
  2. For each window in post-order:
     a. FrameManager.WaitAll()  — wait for THIS window's in-flight frames
        (not global WaitIdle — only wait for this surface's timeline values)
     b. DeferredDestructor.DrainAll()  — flush pending for this surface
     c. Detach RenderSurface  — destroy swapchain
     d. Destroy FrameManager
  
  3. Destroy OS windows (post-order, via WindowManager)
```

**Key difference from miki**: miki uses `device->WaitIdle()` (global stall). This design uses per-surface timeline waits — other windows continue rendering during cascade destruction.
...
|------|-----------|---------------|
| `include/mitsuki/frame/FrameManager.h` | `mitsuki::frame` | Frame pacing: BeginFrame/EndFrame, timeline wait, surface integration |
| `include/mitsuki/frame/FrameContext.h` | `mitsuki::frame` | Per-frame state: index, number, swapchain image, dimensions |
| `include/mitsuki/frame/FrameOrchestrator.h` | `mitsuki::frame` | Multi-window frame orchestration, global deferred destruction |
| `include/mitsuki/frame/SyncScheduler.h` | `mitsuki::frame` | Cross-queue timeline dependency resolution (arbitrary queue-pair, including T→C→G chain) |
| `include/mitsuki/frame/AsyncTaskManager.h` | `mitsuki::frame` | Long-running cross-frame compute tasks (BLAS rebuild, GDeflate decode, GPU QEM) |
| `include/mitsuki/frame/DeferredDestructor.h` | `mitsuki::frame` | Frame-tagged resource destruction queue |
| `include/mitsuki/rhi/TimelineSyncPoint.h` | `mitsuki::rhi` | `{SemaphoreHandle, uint64_t value}` pair |

...
#### Phase E: Executor — DONE

| #    | Item                                                                                             | Spec Section | Status      | Notes                                                                                                                                                                                                                                        |
| ---- | ------------------------------------------------------------------------------------------------ | ------------ | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E-1  | `RenderPassContext` — pass execution interface (handle resolution)                               | §6.3         | **DONE**    | Header + CRTP `Dispatch` via `CommandListHandle::Dispatch`. `ResolveTexture`, `GetTextureView`, `GetBuffer` all functional.                                                                                                                  |
| E-2  | `RenderGraphExecutor::AllocateTransients` — heap pool + placed resource creation                 | §6.1         | **DONE**    | Per-group heap creation via `TransientHeapPool` with cross-frame reuse, `AliasTextureMemory` binding, imported resource passthrough, texture view creation, buffer suballocation. Phase F fully integrated.                                  |
| E-3  | `RenderGraphExecutor::RecordPasses` — barrier emission + pass lambda invocation                  | §6.1         | **DONE**    | Single-threaded path with merged group awareness. Per-pass debug labels (PIX/RenderDoc/NSight). Aliasing barrier emission from `AliasingLayout`. `RGAttachmentInfo`-based load/store.                                                        |
| E-4  | `RenderGraphExecutor::SubmitBatches` — queue submission with sync metadata                       | §6.1         | **DONE**    | Timeline semaphore signal/wait, cross-queue dependency resolution via `SyncScheduler`, pending wait accumulation.                                                                                                                            |
| E-5  | Parallel command recording (Tier1: multi-threaded secondary cmd bufs)                            | §6.2         | **DONE**    | `RecordPassesParallel`: per-thread `CommandPoolAllocator::Acquire`, secondary cmd bufs, primary barrier emission + `CmdExecuteSecondary`. Fallback to single-threaded for small batches.                                                     |
| E-6  | Thread-safety invariants: command pool affinity, per-pass context isolation                      | §6.2.1       | **DONE**    | Per-thread pool acquisition via threadIndex, `std::latch` synchronization, atomic error flag, no shared mutable state in worker lambdas.                                                                                                     |
| E-7  | Async graph execution (render thread offloading)                                                 | §6.2.2       | **DONE**    | `ExecuteAsync` returns `std::future`, `SubmitAfterAsync` for Phase 3 on render thread. State reset on calling thread before dispatch.                                                                                                        |
| E-8  | CRTP zero-overhead dispatch path (`DispatchCommands`)                                            | §6.3.1       | **DONE**    | All RHI commands via `CommandListHandle::Dispatch([&](auto& cmd) { ... })` — zero-overhead CRTP dispatch throughout executor.                                                                                                                |
| E-9  | Backend-specific execution (Vulkan dynamic rendering, D3D12 render pass, GL FBO, WebGPU encoder) | §6.4         | **PARTIAL** | Merged group detection + group-level `CmdBeginRendering`/`CmdEndRendering` bracketing. Full backend-specific paths (VK_KHR_dynamic_rendering_local_read, D3D12 render pass tiers) deferred to backend layer. Phase G cross-queue parts DONE. |
| E-10 | ReadbackRing — triple-buffered host-visible ring with latency hiding                             | §6.5.2       | **DONE**    | `ReadbackRing` header + integration points in `RecordPasses` and `RecordPassesParallel` (end-of-batch `RecordTransfers`). Ring accessible via `RenderPassContext`.                                                                           |
| E-11 | Readback buffer as graph resource + barrier sequence                                             | §6.5.1       | **PARTIAL** | Readback accessible via `RenderPassContext::readbackRing`. Auto-barrier generation for readback resources deferred to Phase G-5.                                                                                                             |

#### Phase F: Transient Memory Management — DONE

| #   | Item                                                                      | Spec Section  | Status   | Notes                                                                                                                                                               |
| --- | ------------------------------------------------------------------------- | ------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| F-1 | Heap pool: cross-frame heap reuse with `layoutHash` matching              | §5.6.5        | **DONE** | `TransientHeapPool` with FNV-1a layoutHash, 3-tier matching (exact hash → size-compatible → new alloc), LRU eviction with configurable grace period                 |
| F-2 | Transient buffer suballocator: linear offset packing within single buffer | §5.6.7        | **DONE** | `PrepareBufferSuballocations`: single parent buffer, 256B-aligned linear packing, per-frame offset reset. 15 CreatePlacedResource → 1                               |
| F-3 | Heap defragmentation: over-provisioned heap detection + replacement       | §5.6.8        | **DONE** | `DefragmentIfNeeded`: VRAM pressure / heap proliferation / explicit trigger. Right-sizes over-provisioned heaps, evicts stale entries                               |
| F-4 | D3D12 heap grouping: RT/DS, non-RT, buffers, mixed fallback               | §5.6.2        | **DONE** | `HeapGroupHint` in `MemoryHeapDesc`, `ResourceHeapTier` in `GpuCapabilityProfile`. Tier2 split heaps, Tier1 mixed fallback                                          |
| F-5 | Vulkan heap grouping: by `memoryTypeBits`                                 | §5.6 footnote | **DONE** | `HeapGroupHint` maps to backend-specific heap flags. Vulkan groups by memoryType via groupHint                                                                      |
| F-6 | Aliasing barrier batching: co-locate with acquire barriers at pass start  | §5.6.4        | **DONE** | `EmitAliasingBarriersForPass` batches all aliasing barriers per topo position, co-located with acquire barriers in `RecordPasses`. Alignment-aware packing (§5.6.6) |

#### Phase G: Cross-Queue & Async Compute — DONE

| #   | Item                                                             | Spec Section | Status   | Notes                                                                                                                                                                                 |
| --- | ---------------------------------------------------------------- | ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| G-1 | Adaptive async threshold (occupancy-aware feedback, EMA)         | §7.2.1       | **DONE** | `AsyncComputeScheduler` with per-pass EMA (α=0.1), 8-frame cold start, 4-frame hysteresis, `ComputeQueueLevel` A-D detection. Integrated into compiler Stage 4 `AssignQueues`.        |
| G-2 | Vulkan QFOT emission: release on src queue, acquire on dst queue | §5.5         | **DONE** | Compiler Stage 6 emits split release/acquire for EXCLUSIVE images (`DetermineQfotStrategy`). Buffers use CONCURRENT (no QFOT). `QfotStrategy` enum + `DetermineQfotStrategy` utility. |
| G-3 | D3D12 cross-queue state transitions                              | §5.5         | **DONE** | `DetermineQfotStrategy` returns `None` for non-Vulkan backends — single barrier at dst pass.                                                                                          |

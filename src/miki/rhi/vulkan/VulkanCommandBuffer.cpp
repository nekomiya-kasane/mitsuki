...
namespace miki::rhi {
    ...
    namespace {
        ...
        // Compile-time proof: miki enum bit values == Vulkan synchronization2 values.
        // If any static_assert fires, RhiEnums.h is out of sync with the Vulkan spec.
        ...
        static_assert(
            uint32_t(PipelineStage::TessControlShader) == VK_PIPELINE_STAGE_2_TESSELLATION_CONTROL_SHADER_BIT
        );
        static_assert(
            uint32_t(PipelineStage::TessEvalShader) == VK_PIPELINE_STAGE_2_TESSELLATION_EVALUATION_SHADER_BIT
        );
        static_assert(uint32_t(PipelineStage::GeometryShader) == VK_PIPELINE_STAGE_2_GEOMETRY_SHADER_BIT);
        ...
        static_assert(
            uint32_t(PipelineStage::ColorAttachmentOutput) == VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT
        );
        ...
        static_assert(uint32_t(PipelineStage::CommandPreprocess) == VK_PIPELINE_STAGE_2_COMMAND_PREPROCESS_BIT_EXT);
        static_assert(
            uint32_t(PipelineStage::ConditionalRendering) == VK_PIPELINE_STAGE_2_CONDITIONAL_RENDERING_BIT_EXT
        );
        ...
        static_assert(uint32_t(PipelineStage::FragmentDensity) == VK_PIPELINE_STAGE_2_FRAGMENT_DENSITY_PROCESS_BIT_EXT);
        static_assert(uint32_t(PipelineStage::TransformFeedback) == VK_PIPELINE_STAGE_2_TRANSFORM_FEEDBACK_BIT_EXT);
        static_assert(
        ...
        static_assert(uint32_t(AccessFlags::CommandPreprocessRead) == VK_ACCESS_2_COMMAND_PREPROCESS_READ_BIT_EXT);
        static_assert(uint32_t(AccessFlags::CommandPreprocessWrite) == VK_ACCESS_2_COMMAND_PREPROCESS_WRITE_BIT_EXT);
        static_assert(
            uint32_t(AccessFlags::ConditionalRenderingRead) == VK_ACCESS_2_CONDITIONAL_RENDERING_READ_BIT_EXT
        );
        ...
        // Zero-cost casts — correctness guaranteed by static_assert above.
        constexpr auto ToVkPipelineStageFlags2(PipelineStage s) -> VkPipelineStageFlags2 {
            return static_cast<VkPipelineStageFlags2>(s);
        }
        constexpr auto ToVkAccessFlags2(AccessFlags a) -> VkAccessFlags2 {
            return static_cast<VkAccessFlags2>(a);
        }

/** @file WebGPUCommandBuffer.h
 *  @brief WebGPU / Dawn (Tier 3) backend command buffer — CRTP implementation.
 *
 *  WebGPUCommandBuffer inherits CommandBufferBase<WebGPUCommandBuffer> and
 *  implements all *Impl methods using Dawn's webgpu.h C API.
 *
 *  WebGPU uses deferred command recording: commands are recorded into a
 *  WGPUCommandEncoder, then finalized into a WGPUCommandBuffer at End().
 *  Render/compute passes use WGPURenderPassEncoder / WGPUComputePassEncoder.
 *
 *  This header is only included when MIKI_BUILD_WEBGPU=1.
 *  Namespace: miki::rhi
 */
#pragma once
...
namespace miki::rhi {
    ...
    class WebGPUCommandBuffer : public CommandBufferBase<WebGPUCommandBuffer> {
       public:
        WebGPUCommandBuffer() = default;

        void Init(WebGPUDevice* device, CommandBufferHandle handle) {
            device_ = device;
            commandBufferHandle_ = handle;
        }
        ...
        // --- Synchronization (implicit on WebGPU — no-op) ---
        ...
        // --- Secondary (executeBundles on WebGPU) ---
       ...
       private:
        ...
        CommandBufferHandle commandBufferHandle_;
        WGPUCommandEncoder encoder_ = nullptr;
        WGPURenderPassEncoder renderPass_ = nullptr;
        WGPUComputePassEncoder computePass_ = nullptr;
        WGPUCommandBuffer finishedCommandBuffer_ = nullptr;  // Result of wgpuCommandEncoderFinish
        PipelineHandle currentPipeline_;
        WGPUIndexFormat currentIndexFormat_ = WGPUIndexFormat_Uint32;
        bool inRenderPass_ = false;
        bool inComputePass_ = false;

        // Helper: end any active pass encoder before transfer/barrier operations
        void EndActivePass();

        friend class WebGPUDevice;  // For Submit to access finishedCommandBuffer_
    };
